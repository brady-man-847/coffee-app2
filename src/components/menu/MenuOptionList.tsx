...
export default function MenuOptionList({ idx, data }: Props) {
  ...
  const handleRadioOptionChange = (_e: ChangeEvent<HTMLInputElement>, value: string): void => {
    dispatch({ type: 'SET_ORDER', order: { ...order, optionValueList: _.defaults({ [idx]: value }, { ...order.optionValueList }) } });
  };
  ...
  return (
    <Box>
      <FormControl>
        {data?.length === 1 ? (
          <FormControlLabel value={data[0].code} control={<Checkbox onChange={handleCheckOptionChange} />} label={data[0].name} />
        ) : (
